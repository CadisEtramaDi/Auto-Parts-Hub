              ...
              <tr>
                ...
                <td>
                  <form method="POST" action="{{ route('cart.item.remove', ['rowId' => $item->rowId]) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="remove-cart btn btn-link">
          ...
          <div class="cart-table-footer">
            @if(Session::has('coupon'))
              <div class="position-relative bg-body">
                <input class="form-control" type="text" value="{{ Session::get('coupon')['code'] }} - Applied" readonly>
                <a href="{{ route('cart.coupon.remove') }}" class="btn-link fw-medium position-absolute top-0 end-0 h-100 px-4 text-decoration-none">REMOVE COUPON</a>
              </div>
            @else
